...
[ -n "$OUT" ] || { echo "usage: $0 [--minimal|--from-host] OUT.img" >&2; exit 2; }
...
if [ "$MODE" = host ]; then
    ...
    ln -s usr/bin "$ST/bin" 2>/dev/null || true
fi
...
if command -v mkfs.erofs >/dev/null; then
    ...
    tar -C "$ST" -cf "$OUT.tar" .
    echo "no mkfs.erofs/mksquashfs — wrote $OUT.tar" >&2
    echo "install: pacman -S erofs-utils  or  squashfs-tools" >&2
fi
